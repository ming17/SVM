...
for testSet in range(1,6): 
	...
	for class1 in range(1, 40):
		...
		for class2 in range(class1+1, 41):
			...
			Y = np.dot(yVals[:, None],yVals[None, :])
			K = np.dot(xVals,xVals.T) + 1
			P = matrix(Y * np.linalg.matrix_power(matrix(K), DEGREE))
	...
	print ("Your accuracy for polynomial kernel test number " + str(testSet) + " with degree " + str(DEGREE) + " was " + str(numCorrect/numTested))
	accuracy += numCorrect/numTested*100

print("Your average polynomial kernel degree " + str(DEGREE) + " test accuracy was: " + str(accuracy/5) + "%")
